...
The following configuration points are available for the ``aiven``
provider:

-  ``aiven:apiKey`` (environment: ``aiven_API_KEY``) - the API key for
   ``aiven``
-  ``aiven:region`` (environment: ``aiven_REGION``) - the region in

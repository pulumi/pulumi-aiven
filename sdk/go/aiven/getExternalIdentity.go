...
func GetExternalIdentityOutput(ctx *pulumi.Context, args GetExternalIdentityOutputArgs, opts ...pulumi.InvokeOption) GetExternalIdentityResultOutput {
	return pulumi.ToOutputWithContext(context.Background(), args).
		ApplyT(func(v interface{}) (GetExternalIdentityResultOutput, error) {
			...
			opts = internal.PkgInvokeDefaultOpts(opts)
			var rv GetExternalIdentityResult
			secret, err := ctx.InvokePackageRaw("aiven:index/getExternalIdentity:getExternalIdentity", args, &rv, "", opts...)
			if err != nil {
				return GetExternalIdentityResultOutput{}, err
			}

			output := pulumi.ToOutput(rv).(GetExternalIdentityResultOutput)
			if secret {
				return pulumi.ToSecret(output).(GetExternalIdentityResultOutput), nil
			}
			return output, nil
